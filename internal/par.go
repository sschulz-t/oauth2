...
// RetrievePARRequestUri performs an OAuth 2.0 Pushed Authorization Request (PAR) by sending
...
func RetrievePARRequestUri(ctx context.Context, clientID, clientSecret, PARURL string, v url.Values, authStyle AuthStyle, styleCache *AuthStyleCache) (*PARResponse, error) {
	...
	req, err := newTokenRequest(PARURL, clientID, clientSecret, v, authStyle)
	if err != nil {
		return nil, err
	}
	...
	if err != nil && needsAuthStyleProbe {
		...
		req, _ = newTokenRequest(PARURL, clientID, clientSecret, v, authStyle)
		parResponse, err = doPARRoundTrip(ctx, req)
	}
...
func doPARRoundTrip(ctx context.Context, req *http.Request) (*PARResponse, error) {
	...
	fmt.Println("R CODE ", r.StatusCode)
